...
public class BoardController {
	...
	public String getBoardOne(@PathVariable Integer id, Model model) {
		...
		if(id==1) {
			model.addAttribute("board", new Board(1, "스프링1강", "컨트롤러 배우기")); // 0번지에 1 있으니
		}
		if(id == 2) {
			model.addAttribute("board", new Board(2, "스프링2강", "템플릿 엔진 배우기"));
		}
		if(id == 3) {
			model.addAttribute("board", new Board(3, "스프링3강", "디비 연결 하기"));
		}
